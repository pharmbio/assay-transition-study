...
public class ClassificationCLI {
	...
	public static class CLIParams {
		...
		@Parameter(names= {"--seed", "-se"}, required = true)
		long seed;

		@Parameter(names = {"--log"}, description = "Extra logfile for the grid-search steps (optional)")
		String logfileName;

		@Parameter(names = {"--help"}, help=true)
		boolean help = false;

		@Parameter(names = {"--a1-size"}, description = "Fixed A1 (A old) size (negative value = all data!)")
		int a1Size = -1;
	...
	public static void main(String[] args) throws Exception {
		...
		if (params.a2Size == null || params.a2Size.isEmpty())
			params.a2Size = Arrays.asList(1000000);
		...
		try (BufferedWriter resWriter = new BufferedWriter(new FileWriter(params.resultsFile))){
			...
			for (int a2 : params.a2Size) {
				...
				case 1:
					// A2
					System.out.println("Running A2 with seed: " + params.seed + ", ccpFolds=" + params.ccpFolds);
					ss = new A2SS(params.ccpFolds);
					break;
				case 2:
					// Assay transition
					System.out.println("Running ASSAY-TRANSITION with seed: " + params.seed +  ", ccpFolds=" + params.ccpFolds);
					ss = new ATSS(params.ccpFolds);
					break;
				case 3: 
					// A1 + A2 mix
					System.out.println("Running A1+A2 with seed: " + params.seed + ", ccpFolds=" + params.ccpFolds);
					ss = new A1A2MixSS(params.ccpFolds);
					break;
				case 4:
					// A1=normal, A2 = calibration exclusive
					System.out.println("Running A1=normal, A2=CALIBRATION EXCLUSIVE with seed: " + params.seed + ", ccpFolds=" + params.ccpFolds);
					ss = new A2CE_SS(params.ccpFolds);
					break;
				case 5:
					// A1 
					System.out.println("Running A1 with seed: " + params.seed + ", ccpFolds=" + params.ccpFolds);
					ss = new A1SS(params.ccpFolds);
					break;
				case 6:
					System.out.println("Running ICP with seed: " + params.seed);
				...
				try {
					runner.run(prob, params.logfileName, ss, a2, resWriter, addHeader);
				} catch (Exception e) {
